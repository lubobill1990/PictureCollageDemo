...
struct b2Vec2
{
	...
	b2Vec2 operator /(float32 number){
		return b2Vec2(this->x/number,this->y/number);
	}
...
/// 以竖列的两个向量来表示和存储，就像数学中矩阵一样，也是一个个竖着写的向量组成
struct b2Mat22
{
	...
	/// 相当于使用一个角度变换出一个二维旋转矩阵
	/// explicit表示不希望进行默认的隐式转换
	/// 如：
	/// b2Mat22 a=2
	/// 如果下面函数没有explicit，则该语句可以调用下面的函数生成a
	...
	/// a rotation matrix).
	/// 把该矩阵当作一个选择矩阵，得到旋转的角度，与set(float32 angle)对应
...
{
	return fabs(a.x- b.x)<b2_epsilon && fabs(a.y - b.y)<b2_epsilon;
}
