...
void b2World::DrawDebugData()
{
	...
	{
		for (b2Body* b = m_bodyList; b; b = b->GetNext())//对每一个body
		{
			const b2Transform& xf = b->GetTransform(); //获取body在world中的位置和角度
			for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext()) //对body中的每个Fixture
			{
				if (b->IsActive() == false)
				{
				...
				{	enum cat{outer=0x0002,inner=0x0004,bound=0x0008};
					if (*((cat*)(f->GetUserData()))==inner)
					{
						DrawShape(f, xf, b2Color(0.3f, 0.7f, 0.7f));
					}else
